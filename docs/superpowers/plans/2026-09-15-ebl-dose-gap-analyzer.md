...
### Task 25: 닫힌 갭과 빗나간 ROI를 구별할 수 없다는 사실을 인정한다

Task 23/24 리뷰 결과 중 **틀린 숫자나 틀린 결론을 낳는 것들**이다.

**1) `closed_doses()`가 평탄 금속 위의 ROI를 "갭이 닫혔다"로 보고한다 (가장 중요).**

내가 승인한 규칙은 "유효 0 + short > 판정보류"였고, 근거는 "ROI를 엉뚱한 데 놓으면
전부 `no_edge`가 나므로 걸러진다"였다. **그 전제가 틀렸다.** `classify_line`이 대비를
먼저 보기 때문에 평탄 금속에는 `no_edge`가 아니라 `short`가 난다. 컨트롤러 실측
(갭은 x≈256, ROI를 왼쪽 금속 위에 놓음):

```
ROI x0= 30: {'short': 301}
ROI x0= 80: {'short': 298, 'multi_edge': 2, 'sub_resolution': 1}
ROI x0=130: {'short': 300, 'sub_resolution': 1}
```

그래서 빗나간 ROI가 `전 구간 short (301/301 라인)`으로 리포트에 찍히고 곡선에 갭 0
빨간 X가 붙는다. plan이 스스로 "이 도구의 결론이 되는 화면"이라고 부른 곳이다.
사용자는 "이 dose에서 갭이 닫힌다"를 읽는다.

**엔진은 픽셀만 보고 평탄 금속과 닫힌 갭을 구별할 수 없다.** 둘 다 대비가 없다.
그러니 더 똑똑한 판정식을 찾으려 하지 말고, **구별할 수 없다는 사실을 화면에 적는다.**

- [ ] **Step 1: 잘못된 근거를 코드에서 지운다**

`ebl_gap/dataset.py`의 `closed_doses()` docstring과
`tests/test_dataset.py::test_an_unmeasurable_roi_is_not_a_closed_dose`가 둘 다
"ROI를 엉뚱한 데 놓으면 no_edge가 난다"고 적고 있다. 실측과 다르다. 주석을 고치고,
테스트는 남기되 이름과 docstring을 실제로 검사하는 것(전부 `no_edge`인 합성 케이스는
닫힘으로 보지 않는다)에 맞춘다. **거짓 근거를 단 방어 코드는 다음 사람을 속인다.**

- [ ] **Step 2: 곡선과 리포트에 모호함을 적는다 (RED 먼저)**

`ClosedDose`에 판단을 싣지 말고 표현을 바꾼다. 리포트:

```
     300.0 uC : 전 구간 short (301/301 라인, 유효 0)
                갭이 닫혔거나 ROI가 패턴을 벗어났습니다 — 오버레이로 확인하세요
```

곡선의 범례/툴팁도 "전 구간 short"가 아니라 "전 구간 short (확인 필요)"로 적는다.

```python
def test_a_closed_dose_line_says_the_roi_could_also_be_off_the_pattern():
    """평탄 금속 위의 ROI도 전 구간 short를 낸다. 엔진은 둘을 구별할 수 없으므로
    화면이 단정하면 안 된다."""
    ...
    assert "ROI가 패턴을 벗어났습니다" in text
```

- [ ] **Step 3: 세션 전체가 short면 설정을 의심한다**

한 dose가 닫히는 것은 정상이다. **모든** dose가 전 구간 short인 것은 dose test로서
말이 안 된다 — ROI가 잘못 놓였거나 스케일/문턱이 틀렸을 가능성이 훨씬 높다. 세션에
측정된 점이 하나도 없고 닫힌 dose만 있으면 한 번 말한다.
...
`Session`에 `session_warnings()`를 두고 GUI 상태 표시줄과 리포트 머리에 싣는다.

**2) README의 노트북 경로가 데이터바 보호를 못 받는다.**

`measure_roi`의 `databar_top`이 기본 `None`이고 README 예제가 안 넘긴다. Step 4가
지키려던 바로 그 경로다. 리뷰어 실측: README 형태로 부르면 26.9%의 날조된 short
비율이 나오고, `databar_top`을 넘기면 정상적으로 거부된다.

- [ ] **Step 4: 호출자가 잊을 수 없게 만든다**

`LoadedImage`가 이미 `databar_top`을 들고 있다. README 예제를 고치는 것만으로는
다음 사람이 또 잊는다. `measure_roi`에 `LoadedImage`를 통째로 받는 편의 함수
`measure_loaded(loaded, roi, *, params=None)`를 두고 README가 그것을 쓰게 한다.
`measure_roi`의 서명은 그대로 둔다(정확도 게이트 27곳이 쓴다).

**3) 회전이 데이터바 가드를 무력화한다.**

`roi.y1 >= databar_top`은 **회전 전** 상자로 검사하는데 `extract_profiles`는
`angle_deg`만큼 돌린 사각형을 훑는다. 모서리가 최대 `(width/2)·|sin θ|`행만큼
아래로 내려간다. 리뷰어 실측: `y1=451`, `databar_top=452`에서 각도 0도면 short 0%,
12도면 8.9% — 5% 문턱을 넘어 곡선에 빨간 X가 **날조된 데이터로** 붙는다.

- [ ] **Step 5: 각도가 정해진 뒤에 검사한다**

엔진은 각도를 아는 첫 번째 자리다. 각도 확정 후
`y1 + ceil((roi.width / 2) * abs(sin(angle_rad)))`로 검사한다. 각도 0도에서 기존
동작과 같아야 하므로 기존 테스트가 그대로 통과해야 하고, 12도 케이스가 거부되는
테스트를 더한다.

**4) 설정을 바꾸면 화면 전체가 낡은 값을 들고 있다.**

Task 23이 `(roi, result)` 쌍 불변식을 세웠는데 Task 24가 `(params, result)` 쌍을
새로 만들고 아무것도 유지하지 않는다. 리뷰어 실측: 문턱을 0.50에서 0.30으로 바꾼 뒤
재측정하지 않아도 결과 패널, 오버레이, dose 곡선, 요약 CSV, **오버레이 PNG 내보내기**가
전부 0.50 값을 그대로 들고 있다. 미니 플롯은 0.30 문턱선과 0.50 에지를 한 그림에
섞어 그린다 — `ProfilePlot.show_line`의 주석이 금지한 바로 그것이다.

- [ ] **Step 6: 설정 변경을 ROI 이동과 같이 취급한다**

`roi_changed`가 받는 처리(오버레이 삭제, 프로파일 삭제, 보관함 비우기, 조작 비활성)를
설정 변경도 똑같이 받게 한다. 내보내기도 같은 이유로 거부되어야 한다 — 실험 노트에
"문턱 0.30"이라고 적으면서 0.50으로 잰 그림을 붙이는 일이 없어야 한다.

`_measure_settings_changed`가 새 params로 `analyze_profile`을 다시 돌려 **옛
`LineResult`와 함께** `show_line`에 넘기는 부분을 지운다. 섞인 그림보다 빈 그림이 낫다.

- [ ] **Step 7: 그 상태 메시지가 지워지는 변이를 잡는다 (M24)**

지금은 "측정 설정이 바뀌었습니다" 줄을 지워도 358개가 통과한다. Step 6이 동작을
바꾸므로 그 동작으로 검사한다.

**5) 패널과 곡선의 short 비율 분모가 아직 다르다.**

Task 23이 연산자(`>=`)는 통일했는데 분모는 통일하지 않았다. `stats`는
`n_valid + n_short + n_uncertain`, `dose_plot`은 `n_valid + n_short`를 쓴다
(`DosePoint`에 `n_uncertain`이 없다). 50 valid / 3 short / 47 uncertain이면 패널
3.0%(조용), 곡선 5.66%(빨간 X). 커밋 메시지가 닫았다고 말한 "두 화면이 다른 답을
준다"가 판정보류 라인이 있으면 여전히 열려 있다.

- [ ] **Step 8: `DosePoint`에 `n_uncertain`을 싣고 분모를 맞춘다**

경계값으로 판별력 있는 테스트를 쓴다(M10도 같이 잡힌다: `dataset.py`의
`n_total`에서 `n_valid`를 빼도 지금은 358개가 통과한다).

- [ ] **Step 9: 살아남은 나머지 변이 둘**

- **M32**: 리포트의 닫힌 dose 목록 정렬이 안 잡힌다. 측정된 dose **사이에** 닫힌
  dose가 끼는 케이스가 어느 테스트에도 없다. 300(측정)/400(닫힘)/500(측정) 시리즈로
  순서를 고정한다.
- **M31**: `closed_dose_marks()`의 `item.scene() is not self._plot.scene()` 검사를
  지워도 통과한다. 자기 docstring이 "카운터만 올리는 구현은 통과 못 한다"고 적어 둔
  성질이다. 씬에 안 붙은 항목을 만들어 검사한다.
...
### Task 26: 각도 고정의 범위, 안내 채널, 도구 모음

리뷰의 나머지. 틀린 숫자를 직접 만들지는 않지만 사용자를 잘못된 방향으로 민다.

**1) 각도 고정이 이미지 사이에서 끈적이고 경고를 지운다.**

`select_image`가 스핀박스를 재동기화하지 않아 고정값이 전역으로 남고, 고정한 각도에는
타당성 검사를 건너뛰므로 경고가 아예 없다. 리뷰어 실측: 이미지 0에서 -12도로 고정한
뒤 이미지 1로 가서 측정하면 `-12.000도 -> 61.856 nm`(참값 60.0, 3.09% 부풀림)이
**경고 없이** 나온다.

- [ ] **Step 1: 이미지를 바꾸면 고정을 푼다**

각도는 그 이미지의 성질이다. 다른 이미지로 가면 고정을 해제하고 스핀박스를 그
이미지의 추정값으로 되돌린다. 해제했음을 상태 표시줄에 한 번 알린다.

- [ ] **Step 2: 고정 각도에도 범위 검사는 한다 (M5)**

지금은 `else:`로 통째로 건너뛴다. 고정 자체를 되돌리라는 경고는 무의미하지만,
**범위를 벗어난 고정값**은 여전히 알려야 한다 — 사용자가 -70도를 실수로 넣을 수 있다.
문구를 나눈다: 자동 추정이면 "ROI를 확인하세요", 고정이면 "고정한 각도가
{x}도입니다 — 의도한 값인지 확인하세요". 어느 쪽이든 테스트로 고정한다(지금은
`else:`를 `if True:`로 바꿔도 358개가 통과한다).

**2) 안내가 `record.error` 채널을 탄다.**

`detect_databar_top`은 안내 전용이 맞고 측정을 막지 않는다(리뷰어 확인). 그러나
안내가 `record.error`로 전달돼 리포트에 `오류:`로 찍히고 요약 CSV의 경고 칸에 들어간다.
데이터바가 없는 멀쩡한 FEI 이미지도 아래쪽이 어두우면 걸린다(`ResolutionY == height`면
`databar_top_row`가 `None`이라 휴리스틱이 돈다).

- [ ] **Step 3: `ImageRecord`에 `notes` 채널을 만든다**

`error`는 "이 이미지로 측정할 수 없다"만 쓴다. 안내는 `notes`로 보내고 리포트와 CSV가
다르게 표기한다(`오류:` 대 `참고:`). `record.error`를 읽는 자리를 전부 확인하고
보고서에 적는다.

- [ ] **Step 4: 오검출률을 정하는 두 상수를 고정한다 (M17, M20)**

`DATABAR_CONTRAST_RATIO`와 기준 행의 `np.median`이 둘 다 변이로 안 잡힌다. 음성
테스트가 아래쪽 변화가 전혀 없는 이미지를 써서 문턱을 5배 느슨하게 해도 대가가 없다.
**아래쪽이 어두운 멀쩡한 시료**를 음성 케이스로 넣는다 — 그것이 실제 오검출 대상이다.

**3) 설정 도구 모음이 최소 크기에서 내보내기 동작을 가린다.**

`setMinimumSize(900, 650)`으로 지원한다고 선언한 크기에서 주 도구 모음이 578px 힌트
대비 404px로 눌려 `라인 CSV`, `오버레이 PNG`, `요약 리포트`가 오버플로 뒤로 숨는다.
도달은 가능하지만 이 프로젝트가 이미 테스트를 쓰는 종류의 문제다.

- [ ] **Step 5: 설정을 별도 줄이나 패널로 내린다**

`addToolBarBreak`로 줄을 나누거나 측정 설정을 오른쪽 패널로 옮긴다. 900x650에서 주
도구 모음의 동작이 하나도 가려지지 않는 것을 테스트로 박는다.
